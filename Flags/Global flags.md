# Global Flags
A global `flag` (not to be confused with the generic programming term) is a `bool` which is a global variable the game can access for any purpose. They are saved on the save file and typically have one reserved use, but some can be used temporarilly notably number 400 and number 0 (reserved for debugging). There are 750 global flags available to the game in the `MainManager.instance.flags` array.
