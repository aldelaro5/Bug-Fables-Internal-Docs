...
|15|[JumpSpring](ObjectTypes/JumpSpring.md)|A spring that makes an entity jump up high via [Jump](../EntityControl/EntityControl%20Methods.md#jump) when touched or makes the entity jump straight to a specific point via JumpTo with the option to trigger an inside transition via a `DoorSameMap` present on the map without physically moving the player.|
