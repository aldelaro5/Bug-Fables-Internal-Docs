...
|MainCamera|Camera|Yes|No|The main camera that renders the `Default` layer. See the camera system documentation to learn more|
|GUICamera|Camera|Yes|No|The camera that renders the `UI` layer. See the camera system documentation to learn more|
|globalcamdir|Transform|No|No|The `CamDir` GameObject, check the camera system documentation to learn more|
|camtarget|Transform|No|No|If set to not null, the camera will try the look at this transform and follow them, check the camera system documentation to learn more|
...
|camoffset|Vector3|No|No|The current postion offset the camera is from its looking target. Check the camera system documentation to learn more. The default value is `defaultcamoffset`|
|camangleoffset|Vector3|No|No|The current angular offset the camera is from its looking target. Check the camera system documentation to learn more. The default value is `defaultcamangle`|
|camoffset2|Vector3|No|No|A secondary postion offset the camera is from its looking target that gets added to `camoffset`. Check the camera system documentation to learn more. It is used to smoothly offset the camera during player movement|
|camtargetpos|Vector3?|No|No|The position the camera is set to look at or null if the camera isn't set to look at a particular point. Check the camera system documentation to learn more|
|camanglechange|bool|No|No|Indicate whether or not to use `camanglespeed` (if it's true) or `camspeed` (if it's false) as the angle lerp factor when rotating the camera while `map.rotatecam` is false. Check the camera system documentation to learn more|
...
|camspeed|float|No|No|A lerp factor used to move the camera to the desired postion and if `camanglechange` is false, it is also the lerp factor when rotating the camere while `map.rotatecam` is false. Check the camera system documentation for more details. The default value is hardcoded to 0.1|
...
|storeid|int|Yes|No|The flagvar slot to store the listvar option upon confirmation of an ItemList. It should always be 0 unless specified otherwise by a pickitem SetText command. This value isn't automatically reset when the ItemList processing is over|
...
|listtype|int|Yes|No|The listtype of the last ItemList OR the flagvar slot where the output of the current numberprompt will be stored OR the flagstring slot where the output of the current letterprompt will be stored. This value isn't automatically reset when the ItemList processing is over|
...
|skiptext|bool|No|No|Indicate whether or not SetText received an input telling it to stop the letter rendering delays in dialogue mode until the end of the textbox TODO: explain the delimitation better|
...
|diagstring|List<string>|Yes|Yes|The SetText dialogue lines cumulated in order for the backtracking system. See the backtracking system documentation to learn more|
|currentdialogue|int|Yes|Yes|The `diagstring` index of the SetText dialogue line that is being seen for the backtracking system. See the backtracking system documentation to learn more|
...
|tempdiag|string|Yes|Yes|The cumulated text of the current textbox of the current SetText call in dialogue mode. This is always reset to `\|size,X,Y\|` on every SetText call in dialogue mode where `X` and `Y` are the components of the size parameter. This is used for the backtracking system so check its documentation to learn more|
|linebr|float|Yes|Yes|The last observed value of linebreak during a SetText call in dialogue mode updated at the start of the char loop. This is used for the backtracking system to use as the maxoffset parameter to OrganiseLines when adding the current textbox to `diagstring`. Check the backtracking system documentation to learn more|
|fontdsize|float|Yes|Yes|The last observed value of size during a SetText call in dialogue mode updated at the start of the char loop. This is used for the backtracking system to use as the size parameter to OrganiseLines when adding the current textbox to `diagstring`. Check the backtracking system documentation to learn more|
|fontdtype|int|Yes|Yes|The last observed value of fonttype during a SetText call in dialogue mode updated at the start of the char loop. This is used for the backtracking system to use as the fontid parameter to OrganiseLines when adding the current textbox to `diagstring`. Check the backtracking system documentation to learn more|
|backtracking|bool|Yes|Yes|Indicate whether or not the current SetText call in dialogue mode is being backtracked. This is always reset to false on any SetText call in dialogue mode or when processing a next command. Check the backtracking system documentation to learn more|
...
|maxoptions|int|No|No|The total TODO: sure??? amount of options of an ItemList, prompt, numberprompt or letterprompt|
...
|numberprompt|bool|No|No|Indicate whether or not there's a numberprompt or letterprompt in progress. The value of `flagvar[0]` can be used to differentiate between the 2: if it's -555, it's a letterprompt and if it's not, it's a numberprompt TODO: only reset when a prompt command happens ???|
...
|prizeflags|int[]|No|No|The flags flagvar slots bound to each of the 23 prize medals. The value is hardcoded to {13, 17, 18, 19, 20, 21, 25, 33, 34, 36,	44, 45, 46, 48, 51, 52, 57, 61, 63, 30, 31, 64, 65}|
|prizeids|int[]|No|No|The medals id bound to each of the 23 prize medals. The value is hardcoded to {5, 24, 14, 15, 1, 32, 10, 73, 37, 26, 66, 67, 65, 60, 55, 61, 0, 63, 68, 79, 58, 70, 68}|
|prizeenemyids|int[]|No|No|The enemies id bound to each of the 23 prize medals. The value is hardcoded to {2, 0, 24, 3, 15, 31, 46, 49, 54, 69, 76, 42, 41, 51, 34, 0, -1, 72, 36, 97, 96, 40, 90}|
...
|Name|type|Is static?|Is Private?|Description|
|---:|-----|----------|------------|------------|
|discoverymessage|Transform|No|No|Contain the `Discovery` UI GameObject childed to `Prefabs/Objects/logbookicon` that is created on the first RefreshDiscovery calls, but shows up onscreen when `discoveryhud` is above 0.0 (left hidden below the screen otherwise)|
...
|hudvisible|bool|Yes|No|Indicates whether or not the HUD is visible according to ShowHUD. If the value is false when ShowHUD(true) is called, ForceHUD is called and the value is set to true. TODO: unclear what ForceHUD does|
|hudvalue|float[]|Yes|Yes|Set to a hardcoded array on CreateHUD which is {6.5, 7.5, 8.5, 9.5, -6.5, -7.0} This corresponds to each `hud` elements's TODO: explain what they do, seems related to their local y position lerping|
...
|basetp|int|No|No|This is essentially the value of `maxtp`, but without taking into account the medals effects TODO: confirm with a quick test and tp plus. The default value is hardcoded to 10. This is persisted to the save file|
...
|maxstorage|int|No|No|The current maximum quantity of standard items allowed to be in the player's storage. The default value is hardcoded to 35. This is persisted to the save fil|
...
|librarystuff|bool[,]|No|No|A jagged array that tracks all library's collection flag grouped by their type on the first dimension:<ul><li>0: Discovery</li><li>1: Bestiary</li><li>2: Recipe</li><li>3: Record</li><li>4: Area</li></ul>The lengths of the array are hardcoded to 5 (the library types) and 256 (overprovisions each type, the actual amount for each type is in `librarylimit`). The entire jagged array is persisted to the save file|
...
|fps|int|Yes|No|A number that indicates the Application.targetFrameRate configured in the settings. Here are the meanings of the values:<ul><li>0: Application.targetFrameRate of 30</li><li>1: Application.targetFrameRate of 60</li><li>2: Application.targetFrameRate of -1 (unlimited, but it is not possible to configure this value under normal gameplay)</li></ul>This is persistec to config.dat. The default value before loading config.dat is hardcoded to 1 (60 FPS)|
...
|Name|type|Is static?|Is Private?|Description|
|---:|-----|----------|------------|------------|
|flagvar|int[]|No|No|The flagvar array which are global integers used for various purposes. The array always has a length of 70 and the array is persisted to the save file|
|flags|bool[]|No|No|The flags array which are global booleans used for various purposes. The array always has a length of 750 and the array is persisted to the save file|
|regionalflags|bool[]|No|No|The regional flags array which are booleans scoped to the current area used for various purposes. The array is always reset on a UpdateArea call which only happens on MapControl's Start when the `areaid` changed. The array always has a length of 100 and the array is persisted to the save file|
|crystalbflags|bool[]|No|No|A flags array which are global booleans used specifically to track each crystal berries's collection (each booleans tells if the crystal berry was collected or not). The array always has a length of 50 and the array is persisted to the save file|
|flagstring|string[]|No|No|The flagstring array which are global strings used for various purposes. The array always has a length of 15 and the array is persisted to the save file|
...
|roomtransition|bool|Yes|No|A value that indicate whether or not a TransferMap call or a save SetText command processing is in progress. This is used by DoClock to see if it is save to call Ressources.UnloadUnusedAssets where if the value is true, the call won't happen|
...
|minipause|bool|No|No|This value is similar to `pause`, but it set progammatically in a wide variety of situations. It doesn't have all the effects of `pause`, but it features most of them so it allows the game to be in a paused state without needing a complete pause menu TODO: recheck to make sure of the differences|
...
|inevent|bool|No|No|Indicate whether or not an event is in progress TODO: weird usage by CardGame ???|
...
|eventtoss|int|Yes|No|Used to store the `data[1]` value of the caller of a SetText call that processes an additemtoss command which corresponds to an event id of an event to start at the end of the SetText call. TODO: This oddly conflicts with CheckItem because there's an event command anyway with it??? The default value is hardcoded to -1 which means no event id is assigned. The value is reset to -1 once the event was started|
...
|Name|type|Is static?|Is Private?|Description|
|---:|-----|----------|------------|------------|
|itempicked|bool|No|No|Indicate whether or not there's a CheckItem call in progress which controls some logic related to JumpSpring objects and Shop / CaravanBadge interactions|
...
|endata|Entity_Data[]|Yes|No|The contents of `Data/EntityValues` ordered by animid (enum format). See the endata documentation to learn more|
