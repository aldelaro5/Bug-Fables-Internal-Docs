...
These fields are everything that's not part of the [configuration fields](Configuration%20fields.md). It means that they aren't meant to have a value assigned from prefab even some of them are public as it would otherwise cause malfunction or unexpected behaviors or be ignored entirely.

They behave more like regular class fields by representing what the map keeps track of.

## Public
The following fields are public and are therefore part of the map's API:

|Name|Type|Init|Description|Default|
|---:|----|----|----------|-------|
|actualcenter|Vector3|From Start|The origin point of the map for the purpose of positioning the camera when `rotatecam` is true representing the point to LookAt|Always set to `centerpoint` on Start|
|entities|EntityControl[]|From Start (CreateEntities)|The list of NPCControl of this map which is initialised in [CreateEntities](../Init%20methods/CreateEntities.md) from the map entity data of this map|Empty list|
...
|mapflags|bool[]|From Start|A list of flags whose value only live throughout the lifetime of this map. Only contains 10 slots all initialised to false. This system can be used with the [mapflag](../../SetText/Individual%20commands/Mapflag.md) SetText command|A new array of 10 elements, all set to false|
|tempfollowers|List<EntityControl>|Always empty list initially|The current list of entities that is following the player party in the map including `chompy`. Tracked by the [follower system](../Follower%20system.md)|Always empty list initially|
...
|commandlines|string[]|From Start|The list of commands strings read from the matching TextAsset data if `useglobalcommand` is true. More details can be learned on the [GlobalCommand](../../SetText/Related%20Systems/GlobalCommand.md) documentation|Empty array|
