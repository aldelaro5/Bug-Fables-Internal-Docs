...
## `playerdata`'s layout
At its core, `playerdata` is an array. This means that no matter what, a member can only be addressed by its index which starts at 0 and ends at the amount of player party members members - 1.

This by itself is very limiting for varying reasons because the game needs to keep track of the player switching the order and the game also needs to sometime address a member by using an [animid](../Enums%20and%20IDs/AnimIDs.md) instead of a particular position in the formation.

Due to this, 2 fields exists on each [BattleData](Actors%20states/BattleData.md) that covers addressing everyone in every way at least in the overworld (meaning these semantics are only for player party members despite those fields existing for enemy party members). Here's the fields and what they do:
...
These 2 rules are always true, no matter what situation the game is in and no matter how many times the player switches whether the player is in the overworld or in battle.
...
In practice, it means the `trueid` can be thought of as the same than the actual `animid` field in the overworld. In fact, the battleentity.`animid` values originally get set to the `playerdata`'s `trueid` values on [StartBattle](StartBattle.md). Essentially, anything that required a specific animid (such as checking a if a [medal](../Enums%20and%20IDs/Medal.md) is equipped on a member with a particular animid) is best suited for this addressing method.
...
A benefit to this is it simplifies the procedure of [SwitchParty](Battle%20flow/Action%20coroutines/SwitchParty.md) and [SwitchPos](Battle%20flow/Action%20coroutines/SwitchPos.md) because all that's needed is to rotate the `partypointer` elements and update the battleentities accordingly (mainly their position).
...
While the `animid` field isn't as useful as it's essentially the same than `trueid`, it is still possible to address by the rendered [animid](../Enums%20and%20IDs/AnimIDs.md). It works in a similar way than the overworld version, but it uses the battleentity.`animid` instead because the battleentity is now the only one that holds the information about the true rendered animid. In other words, use battleentity.`animid` instead of `animid` for this purpose.
