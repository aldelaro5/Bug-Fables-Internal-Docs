...
- The EXP amount is obtained by performing a BattleControl.[GetEXP](../../../TextAsset%20Data/Enemies%20data.md#exp-logic) call on the `exp`, `fixedexp` and the `animid` (the [enemy](../../../Enums%20and%20IDs/Enemies.md) id) clamped from 0 to instance.`neededexp` unless battleentity is a `hologram` in which case, it's clamped from 0 to 5 instead. This method can change the rewarded exp with a given scaled enemy's `exp` field, check the documentation to learn more
