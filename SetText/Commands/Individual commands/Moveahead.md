...
Instruct the game to start a forcemove on an [Entity data](../../../TextAsset%20Data/Entity%20data.md) where the destination is relative to its current position with a configurable speed, start and stop state.
...
The x offset to move the [Entity data](../../../TextAsset%20Data/Entity%20data.md) to. This must be a valid float or an exception will be thrown.
...
The z offset to move the [Entity data](../../../TextAsset%20Data/Entity%20data.md) to. This must be a valid float or an exception will be thrown.
...
This command is very similar to [Move](Move.md), but it has 2 main differences. The first one is the `y` parameter is ignored, its value will always be 0.0. The second one is the final position to move the [Entity data](../../../TextAsset%20Data/Entity%20data.md) isn't directly given, but rather is the result of adding (`x`,0,`z`) to the current entity's position. This allows to move the entity using a vector relative to its position rather than directly specify the new position.
